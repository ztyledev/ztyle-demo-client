...
const Header = () => {
  ...
  var finalName = filterName.includes("app")
	...
	useEffect(() => {
		if (userInfo) {
			dispatch(getNotifications({ id: userInfo._id, token }))
		}
	}, [dispatch, token, userInfo]);
	
	const { notification } = useSelector(state => state.notification);

	// for search shop option
	...
	const handleSearch = () => {
		...
		navigate(`/shops-search?search=${searchWord}`);
		setsearchWord('');
	}
			...
			<div className="header-content">
				...
				<div className="collapse navbar-collapse justify-content-between">
					...
					<ul className="navbar-nav header-right main-notification">
						...
						{/* Notifications 	   */}		  
						<Dropdown as="li" className="nav-item notification_dropdown " onClick={() => dispatch(readNotifications({ id: userInfo._id, token }))}>
