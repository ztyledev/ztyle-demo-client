...
const Header = () => {
  ...
  var finalName = filterName.includes("app")
	...
	useEffect(() => {
		...
		if (beauticianInfo) {
			setuserName(beauticianInfo.fullName);
			setuserEmail(beauticianInfo.email);
		}
	}, [beauticianInfo]);
	...
	useEffect(() => {
		if (beauticianProfile) {
			if (beauticianProfile.profilePic) {
				setprofilePic(beauticianProfile.profilePic);
			}
			else {
				...
				<div className="collapse navbar-collapse justify-content-between">
					...
					<ul className="navbar-nav header-right main-notification">
						...
						<Dropdown as="li" className="nav-item dropdown notification_dropdown">
						  ...
						  {/* Notifications 	   */}
						  <Dropdown as="li" className="nav-item notification_dropdown " onClick={() => dispatch(readNotifications({ id: beauticianProfile._id, token }))}>  
							 <Dropdown.Toggle variant="" as="a" className="nav-link  ai-icon i-false c-pointer">
								<svg width="20" height="24" viewBox="0 0 20 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								  ...
								  {
									  notification ? // check the existance of notification
