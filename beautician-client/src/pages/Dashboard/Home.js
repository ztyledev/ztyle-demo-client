...
const Home = () => {
	...
	// beautician info redux state
	const { email, token } = useSelector(state => state.auth.beauticianInfo);
	
	// access the user beautician profile
	useEffect(() => {
		dispatch(getBeauticianProfile({ email, token }));
	}, [dispatch, email, token]);
	
	// profile redux state
	const { loading, beauticianProfile, error } = useSelector(state => state.beauticianProfile);
								...
								<div className="card text-center">
									<div className="card-body">
										<div id="radialChart" className="radialChart">
											{<RadialDonut profileCompletion={beauticianProfile ? beauticianProfile.profileCompletion : "0"} />}										
										</div>
										<h2> {beauticianProfile ? beauticianProfile.profileCompletion : "0"} %</h2>
										<span className="fs-16 text-black">Profile Completion</span>

										<div style={{ marginTop: "20px" }}>
											{!beauticianProfile ? <div><h4 className="text-danger">Your Profile is empty</h4><Link to="/form-add-basic-profile" > Click Here to Fill up Your Profile </Link> </div> : ""}
