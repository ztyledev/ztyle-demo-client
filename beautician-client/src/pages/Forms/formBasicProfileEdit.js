...
const FormBasicProfileEdit = () => {
  ...
  useEffect(() => {
    ...
    if (beauticianProfile) {
      ...
      const currentDob = beauticianProfile.dob.substring(0, 10);
      // preset to original values
      setfullName(beauticianProfile.fullName);
      setshopId(beauticianProfile.shopId);
      setmobile(beauticianProfile.mobile);
      setemail(beauticianProfile.email);
      setgender(beauticianProfile.gender);
      setdob(currentDob);
      setposition(beauticianProfile.position);
      setspecialty(beauticianProfile.specialty);
      setyearsOfExperience(beauticianProfile.yearsOfExperience);
      setemploymentStatus(beauticianProfile.employmentStatus);
      setholidaySchedule(beauticianProfile.holidaySchedule);
      setlanguagesSpoken(beauticianProfile.languagesSpoken);
  ...
  const handleSubmit=(e)=>{
    ...
    if (employmentStatus === '') {
     ...
     if (holidaySchedule.length ===0) {
      errorObj.languagesSpoken = 'At least take one holiday';
      error = true;
    }
    ...
    const profileData = { fullName, shopId, mobile, email, gender, dob, position, specialty, yearsOfExperience, employmentStatus, holidaySchedule, languagesSpoken };
        ...
        <div className="col-lg-12">
          <div className="card">
            <div className="card-header">
              <h4 className="card-title">Please Edit Necessary Details</h4>
            </div>
